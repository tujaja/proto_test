...
=end
  ...
  def Paypal.express_checkout(params)
    p '{{{Paypal.express_checkout}}}'

    details = SetExpressCheckoutRequestDetailsType.new

    # 貨幣をJPYにする(必須)
    details.orderTotal = Paypal._jpy_amount(params[:amount])
    # 表示ページの言語を日本語に
    details.localeCode = 'JP'
  ...
  def Paypal.express_checkout_payment(params)
    ...
    # 貨幣をJPYにする(必須) SetExpressCheckoutと一致させる
    details.paymentDetails.orderTotal = _jpy_amount(params[:amount])
    ...
    p '{{{DoExpressCheckoutPaymentReq}}}'; pp req

    p '[[[API DoExpressCheckPayment]]]'
...
=end
  ...
  def Paypal._jpy_amount(amount)
    a = BasicAmountType.new(String(amount))
    a.xmlattr_currencyID = 'JPY'
    return a
  end
